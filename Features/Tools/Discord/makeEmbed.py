...
async def makeEmbed(
    ...
    newEmbed = Embed(title=_Title, description=_Description, url=_Url, colour=_Color)
    ...
    if _Footer_Text == '' and _Footer_IconUrl == '': newEmbed.set_footer(text = await getMoment(_IsoFormat=False))
    else: newEmbed.set_footer(text = _Footer_Text, icon_url = _Footer_IconUrl)
    if _Image != None : newEmbed.set_image(url = _Image)
    if _Thumbnail != None : newEmbed.set_thumbnail(url = _Thumbnail)
    if _Timestamp != None : newEmbed.set_thumbnail(timestamp=_Timestamp)
    return newEmbed
