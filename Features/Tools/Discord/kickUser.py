...
async def kickUser(_Context:Context, _Member:Member, _Reason:str, _LogChannel:Messageable) -> None:
    ...
    if IsARegularCall:
        ...
        if CanBanOrKickMembers or _Member == _Context.author:
            await _Context.send('You can\' kick yourself or people with kick / ban permissions')
            return
    
    
    KickedMessage = ':bat: RIP :bat:' + f'\n<@{_Member.id}>' + ' foi kickado!'
    Date = await getMoment(_IsoFormat=False, _KindOfSeparatorDate="/", _WithHours=False)
    Reason = f'**({Date}) - {_Reason}**'
    newEmbed = await makeEmbed(
        _Title='The hammer has spoken',
        _Description=KickedMessage,
    ...
    if IsARegularCall : await _Member.kick()
    return
