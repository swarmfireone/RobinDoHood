async def validateEmbedFields(
    ...
    """
    Validate the _Fields list for emebed's construction
    Rules: 
    + [0] field_name is string
    + [1] field_text is string
    + [2] field_inline is bool
    """
    if _Fields == None or _Fields == []:
        print('Validate Embed Fields: _Fields None')
        return False
    for field in _Fields:
        ...
        if haveThreeFields:
            ...
            if firstFieldString and secondFieldString and thirdFieldBoolean:
                continue
            print('Validate Embed Fields: failed at iteration ' + str(_Fields.index(field)))
